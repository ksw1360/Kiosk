    ...
    internal class Receipt
    {
        ...
        internal static int SaveAcpt(string yKIHO, string pAT_NO)
        {
            ...
            try
            {
                insu_Knd_Cd = "B01";
                sb.Clear();
                int cnt = CheckPatRegvInfo(yKIHO, pAT_NO);
                if (cnt > 0)
                {
        ...
        internal static int CheckPatRegvInfo(string yKIHO, string pAT_NO)
        {
            int ret = 0;
            ...
            sb.AppendLine($" WHERE YKIHO = '{yKIHO}'");
            sb.AppendLine($"   AND PAT_NO = '{pAT_NO}'");
            sb.AppendLine($"   AND VIST_SN = '{DBCommon.ChkVISTNum(yKIHO, pAT_NO)}'");
            sb.AppendLine($"   AND DEL_YN = FALSE");
